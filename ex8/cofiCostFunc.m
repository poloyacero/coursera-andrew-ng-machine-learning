...
f_cost = (X * Theta' - Y);
%J = sum((f_cost .^ 2)(R == 1)) / 2;

%collaborative filtering cost function with regularization
J = sum(sum((f_cost .^ 2)(R == 1))) / 2 + lambda / 2 .* sum(sum(Theta.^2)) + lambda / 2 .* sum(sum(X.^2));

%collaborative filtering gradient without regularization
%X_grad = ((f_cost) .* R) * Theta;
%Theta_grad = ((f_cost) .* R)' * X;

%collaborative filtering gradient with regularization
X_grad = ((f_cost) .* R) * Theta + lambda .* X;
Theta_grad = ((f_cost) .* R)' * X + lambda .* Theta;
